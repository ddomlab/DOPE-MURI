...
def _group_columns(group: str):
    """The catalyst block: its one-hot columns, plus any numeric column."""
    def select(columns):
...
def _reagent_columns(group: str):
    def select(columns):
        return [c for c in columns
...
def feature_groups(grouping: str, prepared: dict) -> dict:
    ...
    if grouping == "catalyst_reagents":
        return {"fp_catalyst": _group_columns(group),
                "fp_conditions": _reagent_columns(group)}
    if grouping == "per_field":
        groups = {"fp_catalyst": _group_columns(group)}
...
def effective_config(cfg: dict, model: str) -> dict:
    ...
    Precedence, lowest to highest: configs/default.json gp block, then this
    model's override, then the CLI --epochs flag (cli.py already wrote that into
...
def run_task(bundle, runs, model: str, method: str, cfg: dict) -> Path:
    ...
    X = feature_frame(reactions, model, prepared)
    ...
    preprocessor = _preprocessor(X, model, prepared)
