...
# Encoded input count recorded in inputs/model_table.csv:
# 5 catalysts + 4 bases + 3 cosolvents + 12 cores + 10 monomers.
WIDTHS = {"catalyst_ohe": 34}
...
def test_encoded_width(bundle, model):
    reactions, prepared = bundle
    frame = feature_frame(reactions, model, prepared)
    pre = _preprocessor(frame, model, prepared)
    assert pre.fit_transform(frame).shape[1] == WIDTHS[model]


def test_every_input_is_one_hot(bundle):
    """There is no computed chemistry in this screen, so there is no numeric block."""
    reactions, prepared = bundle
    numeric, categorical = model_columns("catalyst_ohe", prepared)
    assert numeric == []
    frame = feature_frame(reactions, "catalyst_ohe", prepared)
    names = _preprocessor(frame, "catalyst_ohe", prepared).fit(frame).get_feature_names_out()
...
def test_held_out_catalyst_encodes_to_zeros_not_a_crash(bundle):
    ...
    encoder = _preprocessor(frame, "catalyst_ohe", prepared).fit(frame.iloc[train])
