...
def collect(runs: str | Path) -> dict[str, pd.DataFrame]:
    ...
    for task in tasks:
        ...
        frame = pd.read_csv(task / "predictions.csv", keep_default_na=False)
        frame["y_true"] = pd.to_numeric(frame["y_true"])
        frame["y_pred"] = pd.to_numeric(frame["y_pred"])
    ...
    # Named lolo_by_group, not lolo_by_ligand/lolo_by_catalyst: the engine serves
    # both and one filename means the report code needs no per-dataset branch.
    by_group = (
        pd.DataFrame([{"model": model, "group_value": value, **_pooled(part)}
                      for (model, value), part in lolo.groupby(["model", "reference_group"])])
        if len(lolo) else pd.DataFrame()
    )
    return {"summary": pd.DataFrame(summary), "predictions": predictions,
            "lolo_by_group": by_group}
