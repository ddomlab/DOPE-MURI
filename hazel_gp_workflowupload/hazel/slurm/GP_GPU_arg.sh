...
# Slurm needs a typed GPU request. Swap the type to whatever your allocation
# actually offers; the LSF variant expresses this as a select[] list.
gpu_request="gpu:a100:1"
# Chain collection after every training job succeeds. Set to 0 for a partial
# or selected submission, then submit GP_collect_CPU_arg.sh by hand.
submit_collection=1
...
#SBATCH --output="${output_dir}/${model}_task${task_id}_GPU.out"
#SBATCH --error="${output_dir}/${model}_task${task_id}_GPU.err"
...
echo "Submitted ${#job_ids[@]} GPU training jobs as ${job_name}."

if [[ "$submit_collection" == 1 ]]; then
    # If Slurm rejects a dependency list this long, name the collection job
    # ${job_name} instead and use --dependency=singleton.
