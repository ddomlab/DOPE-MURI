...
# Submit one CPU job per task. A task is one (feature section, evaluation
# method) pair, and its folds run inside that job -- 1 section x 3 methods =
# 3 jobs, 15 GP fits (5 LOLO + 5 + 5). The task count is read from
...
# CPU fallback: same environment, no GPU requested.
walltime="02:00:00"
...
# This screen is small: 1320 wells and 34 one-hot inputs, so the largest fit is a
# 1105x1105 Cholesky (LOLO with Aphos G3 held out). Every task is 5 folds. The
# default below is deliberately loose -- run with pilot=1 once and size it from
# the measured time before trusting it.
...
#SBATCH --output="${output_dir}/task_${task_id}_CPU.out"
#SBATCH --error="${output_dir}/task_${task_id}_CPU.err"
