"""Export a Cernak run into the schema the review notebook reads.

Ported from gp_collab_rxnpredict/gpc/export_hazel.py so the collected tables
have the same shape in all three projects and the reviews are comparable.
The group here is the catalyst; the "ligand" column name is kept because
gpc/results.py and the notebooks expect it.
...
HAZEL_METHOD_NAMES = {"iid_stratified_5": "iid_matched",
...
def export(bundle, runs, out=None, hazel_method_names: bool = False) -> Path:
    ...
    # This bundle ships without manifest.json (it is training-only); fall back to
    # the ids recorded in inputs/config.json so the collection is still traceable.
    manifest_path = bundle / "manifest.json"
    manifest = (read_json(manifest_path) if manifest_path.exists()
                else {"config_hash": prepared.get("dataset", "cernak"),
